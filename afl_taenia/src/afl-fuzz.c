...
// Taenia {
      "  -a            - calibrate only, used to see where the given inputs lead to.\n\n"
// } Taenia
 ...
 * while ((opt = getopt(argc, argv,
  ...
  while ((opt = getopt(argc, argv,
                       "+i:I:o:f:m:t:T:adnCB:S:M:x:QUWe:p:s:V:E:L:h")) > 0)
// } Taenia
...
// } Taenia
  ...
  // afl-taenia {
  if (getenv("AFL_MAX_INPUT_SIZE"))
  {
      max_input_size = strtol(getenv("AFL_MAX_INPUT_SIZE"), 0, 10);
      if (max_input_size <= 0)
      {
          LOG_ERROR("Wrong value for max_input_size, using default.");
          max_input_size = 512;
      }
  }
  // }
...
// Taenia {
    if (taenia_calibrate_only) {
       SAYF(cYEL "[+] " cRST "Calibration done, exiting.\n");
       exit(1);
    }
// } Taenia
