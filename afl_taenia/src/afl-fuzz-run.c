...
u8 run_target(char** argv, u32 timeout) {
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    ...
    else if(kill_signal == SIGSTOP)
    {
      LOG_ERROR("Process stopped !");
      SAYF(
          "\n\n" cLRD "[!] " cRST "A tracked process was found dead !\n"
          cLBL "[?] " cRST  "AFL stopped fuzzing as the binary ecosystem is now incomplete.\n"
          cLBL "[*] " cRST "To prevent this behaviour, please set the stop_on_process_crash flag to 0 in the configuration file\n\n");
      exit(0);
    }
...
u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  // afl-taenia {
  // We want the input to fit into the targeted buffer.
  u32 nlen = (len >=  max_input_size ? max_input_size:len);
  // }

  write_to_testcase(out_buf, nlen);
  ...
  queued_discovered += save_if_interesting(argv, out_buf, nlen, fault);
